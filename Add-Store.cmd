...
for /f %%i in ('dir /b *NET.Native.Framework*1.6*.* 2^>nul ^| find /i "x86"') do set "Framework16X86=%%i"
for /f %%i in ('dir /b *NET.Native.Framework*1.6*.* 2^>nul ^| find /i "x64"') do set "Framework16X64=%%i"
...
for /f %%i in ('dir /b *NET.Native.Runtime*1.6*.* 2^>nul ^| find /i "x86"') do set "Runtime16X86=%%i"
for /f %%i in ('dir /b *NET.Native.Runtime*1.6*.* 2^>nul ^| find /i "x64"') do set "Runtime16X64=%%i"
...
set "DepStore=%VCLibsX86%,"VCLibsUWPX86",%Framework16X86%,%Runtime16X86%,%Framework6X86%,%Runtime6X86%,%UIXamlX86%"
