# This script will install the Microsoft Store
...
param(
    ...
    # If set, also downloads and installs the HEVC Video Extensions from Device Manufacturer (free video codec needed by some games/apps).
    [switch]$installHevc
)
...
#    .\Install-Microsoft-Store.ps1 -ExistingInstallerFilesPath "C:\Path\To\MSStore Install"
#
...
#
# Also install HEVC Video Extensions from Device Manufacturer:
#    .\Install-Microsoft-Store.ps1 -installHevc
...
# Category IDs for the app packages to fetch from the Windows Update API.
# These can be found via the DisplayCatalog API (the 'WuCategoryId' field in FulfillmentData), e.g.:
#   https://displaycatalog.mp.microsoft.com/v7.0/products?bigIds=<ProductId>&market=US&languages=en-us
$appCategoryIds = @(
    "64293252-5926-453c-9494-2d4021f1c78d",  # Microsoft.WindowsStore
    "9db724c9-966d-4aeb-9d3b-d6b2c77f3de3",  # Microsoft.XboxIdentityProvider (needed by Xbox/Gaming Services and Microsoft account sign-in)
    "f855810c-9f77-45ff-a0f5-cd0feaa945c6"   # Microsoft.DesktopAppInstaller (App Installer, provides winget)
)

if ($installHevc) {
    $appCategoryIds += "5e60c346-7f52-4a6f-98c0-70fef2dd1d01"  # Microsoft.HEVCVideoExtension (HEVC Video Extensions from Device Manufacturer)
}

# Main app packages to look for when filtering the results (latest version of each is kept)
$mainAppNames = @('Microsoft.WindowsStore', 'Microsoft.XboxIdentityProvider', 'Microsoft.DesktopAppInstaller')

if ($installHevc) {
    $mainAppNames += 'Microsoft.HEVCVideoExtension'
}

# Old dependency versions to exclude from downloading (superseded by newer versions also offered by WU)
    ...
    <s:Body>
        <SyncUpdates xmlns="http://www.microsoft.com/SoftwareDistribution/Server/ClientWebService">
            ...
            <parameters>
                ...
                <FilterAppCategoryIds>{1}</FilterAppCategoryIds>
...
try {
    if (-not $NoCheckLatestVersion) {
        ...
        $categoryIdsXml = ($appCategoryIds | ForEach-Object { "<CategoryIdentifier><Id>$_</Id></CategoryIdentifier>" }) -join ""
        $fileListRequestPayload = $fileListXmlTemplate -f $encryptedCookieData, $categoryIdsXml, $currentBranch, $flightRing, $flightingBranchName
        ...
        foreach ($update in $downloadableUpdates) {
            ...
            # PackageRank is Microsoft's own ordering of package versions within an app.
            # It is used instead of comparing Version strings, which breaks when a package
            # switches versioning schemes (e.g. Xbox Identity Provider went from 2017.x to 12.x).
            $packageRank = [int]$update.Xml.Properties.PackageRank
        ...
        try {
            ...
            # 1. Isolate the main app packages and find the latest version of each
            #    (latest = highest PackageRank, see note in the correlation step above)
            $mainPackages = @()
            foreach ($appName in $mainAppNames) {
                ...
                $latestPackage = $candidates |
                    Sort-Object { $_.PackageRank } -Descending |
                    Select-Object -First 1

                if ($latestPackage) {
                    $mainPackages += $latestPackage
                    Write-Host "  -> Found latest ${appName} package: $($latestPackage.FullName)" -ForegroundColor Green
                } else {
                    Write-Warning "Could not find any '$appName' package."
                }
            }
            ...
            $filteredDependencies = $fileIdentityMap.Values |
                Where-Object {
                    ($mainAppNames -notcontains $_.PackageName) -and
                    ($excludedDependencyNames -notcontains $_.PackageName) -and
                    ( ($_.Architecture -eq $systemArch) -or ($_.Architecture -eq 'neutral') )
                }

            # 3. Combine the lists for the final download queue
            $packagesToDownload = @()
            $packagesToDownload += $mainPackages
    ...
    # 3. Get all downloaded package files and separate the main apps from dependencies
    try {
        $allDownloadedFiles = Get-ChildItem -Path $workingDir -File | Where-Object { $_.Extension -in '.appx', '.msix', '.appxbundle', '.msixbundle' }

        $mainPackageFiles = @()
        $dependencyFiles = @()
        foreach ($file in $allDownloadedFiles) {
            if ($mainAppNames | Where-Object { $file.Name -like "$_*" }) {
                # Sorting by name descending ensures the newest version is picked first if multiple exist
                $mainPackageFiles += $file
            } else {
                $dependencyFiles += $file
            }
        }
        $mainPackageFiles = $mainPackageFiles | Sort-Object Name -Descending

        if (-not $dependencyFiles -and -not $mainPackageFiles) {
        ...
        # 5. Install the main application packages last
        if ($mainPackageFiles) {
            Write-Host "Installing the main application(s)..."
            foreach ($mainFile in $mainPackageFiles) {
                Write-Host "  -> Installing $($mainFile.Name)"
                try {
                    Add-AppxPackage -Path $mainFile.FullName
                    Write-Host "     SUCCESS: $($mainFile.BaseName) has been installed/updated." -ForegroundColor Green
                } catch {
                    Write-Error "     FAILED to install $($mainFile.Name). Error: $($_.Exception.Message)"
                }
            }
        } else {
            Write-Warning "No main app packages were found in the download folder."
        }
