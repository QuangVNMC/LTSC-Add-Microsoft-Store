...
# Extra package(s) to install together with Microsoft Store.
# Xbox Identity Provider is needed by Xbox/Gaming Services and Microsoft account sign-in in some Store scenarios.
$extraPackageDownloads = @(
    [PSCustomObject]@{
        PackageName = "Microsoft.XboxIdentityProvider"
        FileName    = "Microsoft.XboxIdentityProvider.AppxBundle"
        Url         = "https://github.com/QuangVNMC/LTSC-Add-Microsoft-Store/releases/download/Xbox/Microsoft.XboxIdentityProvider.AppxBundle"
    }
)
...
try {
    ...
    # --- Download Extra Package(s) ---
    # These packages are not fetched from Windows Update by the Store package query above.
    # They are downloaded from the URLs defined in $extraPackageDownloads.
    If ($noDownload) {
        Write-Host "Skipping extra package download because of -noDownload switch." -ForegroundColor Yellow
        return
    }

    if ($extraPackageDownloads -and $extraPackageDownloads.Count -gt 0) {
        if (-not (Test-Path -Path $workingDir)) {
            New-Item -Path $workingDir -ItemType Directory -Force | Out-Null
        }

        Write-Host "------------------------------------------------------------"
        Write-Host "Downloading extra package(s)..." -ForegroundColor Magenta

        $originalPref = $ProgressPreference
        $ProgressPreference = 'SilentlyContinue'

        foreach ($extraPackage in $extraPackageDownloads) {
            $extraPackagePath = Join-Path $workingDir $extraPackage.FileName

            if (Test-Path $extraPackagePath) {
                Write-Host "  -> Already exists: $($extraPackage.FileName)" -ForegroundColor Yellow
                continue
            }

            Write-Host "  -> Downloading $($extraPackage.PackageName)"
            Write-Host "     From: $($extraPackage.Url)" -ForegroundColor Gray
            Write-Host "     To:   $extraPackagePath"

            try {
                Invoke-WebRequest -Uri $extraPackage.Url -OutFile $extraPackagePath -UseBasicParsing
                Write-Host "     SUCCESS: Download complete." -ForegroundColor Green
            } catch {
                Write-Error "     FAILED to download $($extraPackage.FileName). Error: $($_.Exception.Message)"
            }
        }
    ...
    try {
        ...
        # 5. Install extra package(s) before the main Microsoft Store package
        #    Example: Xbox Identity Provider for Xbox/Gaming Services sign-in support.
        if ($extraPackageDownloads -and $extraPackageDownloads.Count -gt 0) {
            Write-Host "Installing extra package(s)..."
            foreach ($extraPackage in $extraPackageDownloads) {
                $extraPackageFile = $allDownloadedFiles | Where-Object { $_.Name -eq $extraPackage.FileName } | Select-Object -First 1

                if (-not $extraPackageFile) {
                    Write-Warning "Extra package was not found in the download folder: $($extraPackage.FileName)"
                    continue
                }

                Write-Host "  -> Installing $($extraPackageFile.Name)"
                try {
                    Add-AppxPackage -Path $extraPackageFile.FullName
                    Write-Host "     SUCCESS: $($extraPackage.PackageName) has been installed/updated." -ForegroundColor Green
                } catch {
                    Write-Error "     FAILED to install $($extraPackageFile.Name). Error: $($_.Exception.Message)"
                }
            }
        }

        # 6. Install the main Microsoft Store package last
        if ($storePackageFile) {
